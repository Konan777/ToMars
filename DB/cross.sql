/****** Object:  View [pivotparts].[Crm2]    Script Date: 21.06.2019 7:31:28 ******/
SET ANSI_NULLS ON
GO
...
SELECT 
	...
	0 AS [Кандидаты накопительные],
	0 AS [Кандидаты валовые],
	SUM(CASE WHEN A.[StageId] = 644 THEN 1 ELSE 0 END) AS [Нанят накопительные],
	SUM(CASE WHEN A.[StageId] = 647 THEN 1 ELSE 0 END) AS [Мы отказали накопительные],
	SUM(CASE WHEN A.[StageId] = 649 THEN 1 ELSE 0 END) AS [Резерв накопительные],
	SUM(CASE WHEN A.[StageId] = 650 THEN 1 ELSE 0 END) AS [Кандидат отказался накопительные],
	SUM(CASE WHEN A.[StageId] = 653 THEN 1 ELSE 0 END) AS [Смена вакансии накопительные],
	0 AS [Нанят валовые],
	0 AS [Мы отказали валовые],
	0 AS [Резерв валовые],
	0 AS [Кандидат отказался валовые],
...
SELECT 
	...
	0 AS [Кандидат по дате кандидата],
	0 AS [Смена вакансии по дате кандидата],
	SUM(CASE WHEN L.[ActivityId] = 532 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Назначено интервью накопительные],
	SUM(CASE WHEN L.[ActivityId] = 595 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Результаты интервью накопительные],
	SUM(CASE WHEN L.[ActivityId] = 524 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Назначили на оформление накопительные],
	SUM(CASE WHEN L.[ActivityId] = 598 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Результаты оформления накопительные],
	0 AS [Назначено интервью валовые],
	0 AS [Результаты интервью валовые],
	0 AS [Назначили на оформление валовые],
	0 AS [Результаты оформления валовые],
...
SELECT 
	...
	0 AS [Кандидат отказался накопительные],
	0 AS [Смена вакансии накопительные],
	SUM(CASE WHEN A.[StageId] = 644 THEN 1 ELSE 0 END) AS [Нанят валовые],
	SUM(CASE WHEN A.[StageId] = 647 THEN 1 ELSE 0 END) AS [Мы отказали валовые],
	SUM(CASE WHEN A.[StageId] = 649 THEN 1 ELSE 0 END) AS [Резерв валовые],
	SUM(CASE WHEN A.[StageId] = 650 THEN 1 ELSE 0 END) AS [Кандидат отказался валовые],
	SUM(CASE WHEN A.[StageId] = 653 THEN 1 ELSE 0 END) AS [Смена вакансии валовые],
	0 AS [Нанят по дате кандидата],
	0 AS [Мы отказали по дате кандидата],
	0 AS [Резерв по дате кандидата],
	0 AS [Кандидат по дате кандидата],
	...
	SUM(CASE WHEN A.[StageId] = 642 THEN 1 ELSE 0 END) AS [Назначено интервью валовые],
	SUM(CASE WHEN A.[StageId] = 739 THEN 1 ELSE 0 END) AS [Результаты интервью валовые],
	SUM(CASE WHEN A.[StageId] = 638 THEN 1 ELSE 0 END) AS [Назначили на оформление валовые],
...
SELECT 
	...
	0 AS [Кандидат отказался валовые],
	0 AS [Смена вакансии валовые],
	SUM(CASE WHEN A.[StageId] = 644 THEN 1 ELSE 0 END) AS [Нанят по дате кандидата],
	SUM(CASE WHEN A.[StageId] = 647 THEN 1 ELSE 0 END) AS [Мы отказали по дате кандидата],
	SUM(CASE WHEN A.[StageId] = 649 THEN 1 ELSE 0 END) AS [Резерв по дате кандидата],
	SUM(CASE WHEN A.[StageId] = 650 THEN 1 ELSE 0 END) AS [Кандидат по дате кандидата],
	SUM(CASE WHEN A.[StageId] = 653 THEN 1 ELSE 0 END) AS [Смена вакансии по дате кандидата],
	0 AS [Назначено интервью накопительные],
	0 AS [Прошли интервью накопительные],
	0 AS [Назначили на оформление накопительные],
	0 AS [Оформление одобрено накопительные],
...
SELECT 
	...
	0 AS [Назначили на оформление валовые],
	0 AS [Результаты оформления валовые],
	SUM(CASE WHEN L.[ActivityId] = 532 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Назначено интервью по дате кандидата],
	SUM(CASE WHEN L.[ActivityId] = 595 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Результаты интервью по дате кандидата],
	SUM(CASE WHEN L.[ActivityId] = 524 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Назначили на оформление по дате кандидата],
	SUM(CASE WHEN L.[ActivityId] = 598 AND L.[State] <> 'cancel' THEN 1 ELSE 0 END) AS [Результаты оформления по дате кандидата]
	FROM [client].[CrmApplicants] AS A
	LEFT JOIN [client].[CrmPartners] AS P ON A.[PartnerId] = P.[Id]
	LEFT JOIN
	(
...
/****** Object:  View [pivotparts].[IbsToClients]    Script Date: 21.06.2019 7:31:44 ******/
SET ANSI_NULLS ON
GO
...
SELECT   CAST([Date] AS DATE) AS [Дата]
		,[AtomId]
		,[Version]
...
FROM (
		...
		GROUP BY SDA.[CreateDate], PA.[AtomId], PA.[Version]
		...
		LEFT JOIN StartDateApplicants AS SDA ON LA.[PartnerId] = SDA.[PartnerId]
		LEFT JOIN PartnersAtoms AS PA ON LA.[PartnerId] = PA.[Id]
		GROUP BY LA.[Id], SDA.[CreateDate], PA.[AtomId] ,PA.[Version]
